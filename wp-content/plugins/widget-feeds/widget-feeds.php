...
?>
	<ul>
		<li class="feed">最新文章&nbsp;<a href="<?php echo $rss2_url ?>"><?php echo $icon_link ?></a></li>
		<li class="feed"><?php _e('最新留言') ?>&nbsp;<a href="<?php echo $comments_rss2_url ?>"><?php echo $icon_link ?></a></li>
<?php
		//Context-sensitive RSS links
		if ( is_category() ) {
			$rss_link = get_rss_link( 'category', $options['protocol'] );
?>
		<li class="feed"><?php _e('此分類') ?>&nbsp;<a href="<?php echo $rss_link ?>"><?php echo $icon_link ?></a></li>
<?php
		}
		else if ( is_single() ) {
			$rss_link = get_rss_link( 'post', $options['protocol'] );		
?>
		<li class="feed"><?php _e('此篇留言') ?>&nbsp;<a href="<?php echo $rss_link ?>"><?php echo $icon_link ?></a></li>
<?php
		}
		else if ( is_search() ) {
			$rss_link = get_rss_link( 'search', $options['protocol'] );
?>
		<li class="feed"><?php _e('此次搜尋結果') ?>&nbsp;<a href="<?php echo $rss_link ?>"><?php echo $icon_link ?></a></li>
<?php
		}
		else if ( function_exists( 'is_tag' ) && is_tag() ) {
			$rss_link = get_rss_link( 'tag', $options['protocol'] );
?>
		<li class="feed"><?php _e('此標籤') ?>&nbsp;<a href="<?php echo $rss_link ?>"><?php echo $icon_link ?></a></li>
<?php
		}
?>
	</ul>
